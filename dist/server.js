...
var Context = class {
  constructor(text, filePath, workspaceRoot2) {
    this.text = text;
...
var Engine = class _Engine {
  rules = [];
  constructor(rulesPaths2, enableCategories = [], disableCategories = []) {
    const disabled = /* @__PURE__ */ new Set();
    const all = [];
    for (const rp of rulesPaths2) {
  ...
  // ---- エントリポイント ----
  lintText(text, filePath, workspaceRoot2) {
    const ctx = new Context(text, filePath, workspaceRoot2);
    const checkers = {
  ...
  lintFile(filePath, workspaceRoot2) {
    const text = fs.readFileSync(filePath, "utf-8");
    return this.lintText(text, filePath, workspaceRoot2);
  }
};
...
var rulesPaths = splitEnv("WSLSP_RULES");
...
function uriToPath(uri) {
  try {
    return uri.startsWith("file://") ? (0, import_node_url2.fileURLToPath)(uri) : void 0;
  } catch {
    return void 0;
  }
}
connection.onInitialize((params) => {
  const rootUri = params.rootUri ?? void 0;
...
function validate(doc) {
  ...
  const diagnostics = engine.lintText(doc.getText(), filePath, workspaceRoot).map((d) => ({
